...
@torch.no_grad()
def evaluate_single_model(model, loader, device, name, mean, std, is_main):
    model.eval()
    ...
    acc = 100. * correct / total_real_samples
    if is_main:
        print(f" {name}: {acc:.2f}%")
    return acc
...
def train_hesitant_fuzzy(ensemble_model, train_loader, val_loader, num_epochs, lr,
    ...
    for epoch in range(num_epochs):
        ...
        # محاسبه با تعداد واقعی نمونه‌ها (بدون نیاز به all_reduce)
...
# ================== DEVICE SETUP ==================
def setup_device():
    """تنظیم دستگاه برای اجرای Single GPU / CPU"""
...
def main():
    ...
    is_main = True  # در حالت تک کارت، همیشه main هستیم

    if is_main:
        print("="*70)
        print(f"OPTIMIZED FUZZY HESITANT ENSEMBLE TRAINING (Single GPU)")
