...
def final_evaluation_unified(model, test_loader_full, device, save_dir, model_names, args, is_main):
    """
    ارزیابی نهایی یکپارچه - بدون Mixed Precision برای دقت کامل
    """
    ...
    print("Note: Using full precision (float32) for exact accuracy matching\n")
    ...
    with torch.no_grad():
        for i, global_idx in enumerate(tqdm(test_indices, desc="Final Eval")):
            ...
            # بدون autocast - دقت کامل float32
...
def train_hesitant_fuzzy(ensemble_model, train_loader, val_loader, num_epochs, lr,
    ...
    if is_main:
        print("="*70)
        print("Training Fuzzy Hesitant Network (Mixed Precision Enabled)")
        print("="*70)
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in pbar:
            ...
            # Cast back to float32 for accurate metric calculations and statistics
            outputs = outputs.float()
            weights = weights.float()
            memberships = memberships.float()
