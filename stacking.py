در اینجا کد کامل و اصلاح شده فایل `stacking.py` قرار دارد. تغییرات اصلی شامل موارد زیر است:
1.  اضافه کردن `dist.barrier()` پس از ذخیره مدل برای جلوگیری از خطای همگام‌سازی در محیط چند GPU.
2.  پیاده‌سازی کامل تابع `evaluate_ensemble_final_ddp` برای تولید گزارش متنی دقیق (TXT) و فایل JSON درخواست شده.
3.  مدیریت خطا برای بارگذاری مدل در محیط توزیع‌شده.

این کد را می‌توانید جایگزین فایل قبلی کنید:
