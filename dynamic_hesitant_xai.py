...
@torch.no_grad()
def evaluate_single_model(model: nn.Module, loader: DataLoader, device: torch.device, name: str, rank: int, mean: tuple, std: tuple) -> float:
    model.eval()
...
def main():
    ...
    if is_main:
        ...
        for i, model in enumerate(base_models):
            acc = evaluate_single_model(model, test_loader, device, f"Model {i+1} ({MODEL_NAMES[i]})", rank, MEANS[i], STDS[i])
    ...
    if is_main:
        print("\n" + "="*70)
        print("COMPUTING GRAD-CAM FOR INDIVIDUAL MODELS")
        print("="*70)
        ...
        try:
            ...
            print("Test loader is empty. Skipping Grad-CAM.")
        else:
            num_samples = 5
            ...
            for i, model in enumerate(base_models):
                print(f"Computing Grad-CAM for Model {i+1} ({MODEL_NAMES[i]})")
                target_layer = 'layer4.2.conv3'  # Adjust if necessary for your ResNet architecture
                gradcam = GradCAM(model, target_layer)
                norm = transforms.Normalize(mean=MEANS[i], std=STDS[i])
                ...
                for j in range(num_samples):
                    ...
                    plt.imshow(cam.cpu().numpy(), cmap='jet', alpha=0.4)
                    plt.title(f"Sample {j+1} | Label: {labels[j]} | Pred: {pred_label}")
                    plt.axis('off')
                    save_path = os.path.join(gradcam_dir, f"gradcam_model{i+1}_sample{j+1}.png")
