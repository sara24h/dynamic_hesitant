...
import cv2  # Required for applying colormap
...
class GradCAM:
    ...
    def __call__(self, x, class_idx=None):
        ...
        # Global average pooling the gradients over the channels
...
def visualize_and_save_cam(img, cam, title, save_path):
    """
    Visualizes and saves the Grad-CAM output.
    img: The original input image tensor (C, H, W).
    cam: The Grad-CAM heatmap numpy array (H, W).
    title: Title for the plot.
    save_path: Path to save the generated image.
    """
    # Denormalize the image for visualization
    # Assuming ImageNet normalization for display purposes
...
def main():
    ...
    if is_main:
        ...
        # We will use the first image in the batch for visualization
        input_image = images[0].unsqueeze(0).to(device)
        original_image = images[0].cpu()

        # --- Scenario 1: Grad-CAM for each individual model ---
        print("\nGenerating Grad-CAM for individual models...")
        for i, model in enumerate(base_models):
            # The target layer is usually the last convolutional block in ResNet
            grad_cam = GradCAM(model, target_layer_name='layer4')
            
            # We need to normalize the image for the model
            normalized_input = ensemble.module.normalizations(input_image, i)
            
            # Generate CAM
            heatmap = grad_cam(normalized_input)
            
            # Visualize and save
            title = f"Grad-CAM for Model {i+1} ({MODEL_NAMES[i]})"
            save_path = os.path.join(grad_cam_output_dir, f"individual_model_{i+1}_{MODEL_NAMES[i]}.png")
            visualize_and_save_cam(original_image, heatmap, title, save_path)
        
        # --- Scenario 2: Grad-CAM for the ensemble's output ---
        print("\nGenerating Grad-CAM for the ensemble's output...")
        ensemble.eval()
        with torch.no_grad():
            # Get the weights for the sample image from the ensemble
            _, weights, _, _ = ensemble(input_image, return_details=True)
        
        # Find the index of the model with the highest weight
        dominant_model_idx = weights[0].argmax().item()
        dominant_model = base_models[dominant_model_idx]
        
        print(f"Dominant model for this sample: Model {dominant_model_idx+1} ({MODEL_NAMES[dominant_model_idx]}) with weight {weights[0, dominant_model_idx]:.4f}")

        # Generate Grad-CAM for the dominant model
        grad_cam = GradCAM(dominant_model, target_layer_name='layer4')
        normalized_input = ensemble.module.normalizations(input_image, dominant_model_idx)
        heatmap = grad_cam(normalized_input)

        # Visualize and save
        title = f"Grad-CAM for Ensemble (Dominant: Model {dominant_model_idx+1})"
        save_path = os.path.join(grad_cam_output_dir, "ensemble_dominant_model.png")
        visualize_and_save_cam(original_image, heatmap, title, save_path)
