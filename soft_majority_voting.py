...
class SoftVotingEnsemble(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_details: bool = False):
        ...
        # Stack probabilities: (Batch, NumModels, 1)
        stacked_probs = torch.cat(probs_list, dim=1)
        
        # Soft Voting: Average the probabilities
        avg_probs = stacked_probs.mean(dim=1) # Shape: (Batch, 1)
        
        # Final decision based on threshold (0.5)
        final_output = (avg_probs > 0.5).long().float()

        if return_details:
            batch_size = x.size(0)
            # Uniform weights (for compatibility with analysis)
            weights = torch.ones(batch_size, self.num_models, device=x.device) / self.num_models
            
            # Dummy memberships (not used in simple Soft Voting but kept for signature)
            dummy_memberships = torch.zeros(batch_size, self.num_models, 3, device=x.device)
            
            # Return individual model probabilities instead of votes
            individual_probs = stacked_probs.squeeze(2) # (Batch, NumModels)
            
            return final_output, weights, dummy_memberships, individual_probs

        return final_output, stacked_probs
...
def evaluate_ensemble_final_ddp(model, loader, device, name, model_names, is_main=True):
    ...
    # For Soft Voting stats: Accumulate probabilities
    # Sum of probabilities per model
    prob_distribution = torch.zeros(len(model_names), device=device)
    sample_count_per_model = 0 # To calculate average later

    if is_main:
        print(f"\nEvaluating {name} set (Soft Voting)...")
        
    for images, labels in tqdm(loader, desc=f"Evaluating {name}", disable=not is_main):
        images, labels = images.to(device), labels.to(device)
        # outputs: final soft decision (0 or 1)
        # probs: individual probabilities (0.0 to 1.0) [Batch, NumModels]
        outputs, weights, _, probs = model(images, return_details=True)
        ...
        # probs shape: (Batch, NumModels)
        prob_distribution += probs.sum(dim=0) # Sum over batch -> shape (NumModels)
        sample_count_per_model += probs.size(0)

    stats = torch.tensor([total_correct, total_samples], dtype=torch.long, device=device)
    dist.all_reduce(stats, op=dist.ReduceOp.SUM)
    dist.all_reduce(prob_distribution, op=dist.ReduceOp.SUM)
    
    # Sync sample count
    sample_count_tensor = torch.tensor(sample_count_per_model, dtype=torch.long, device=device)
    ...
    if is_main:
        ...
        # Calculate average probability per model
        avg_probs = prob_distribution.cpu().numpy() / global_sample_count

        print(f"\n{'='*70}")
        print(f"{name.upper()} SET RESULTS (Soft Voting)")
        print(f"{'='*70}")
        print(f" → Accuracy: {acc:.3f}%")
        print(f" → Total Samples: {total_samples:,}")
        
        print(f"\nAverage Probability Distribution (Mean Confidence):")
        for i, mname in enumerate(model_names):
            # Probability > 0.5 implies Real, < 0.5 implies Fake
            p_val = avg_probs[i]
           倾向 = "Real" if p_val > 0.5 else "Fake"
            print(f"  {i+1:2d}. {mname:<25}: {p_val:.4f} ({倾向})")
...
def generate_visualizations(ensemble, test_loader, device, vis_dir, model_names,
    ...
    if not is_main:
        return
    print("="*70)
    print("GENERATING VISUALIZATIONS (Soft Voting)")
    print("="*70)
    ...
    for idx, (image, _) in enumerate(vis_loader):
        ...
        # Get prediction
        with torch.no_grad():
            outputs, weights, _, probs = ensemble(image, return_details=True)
        pred = int(outputs.squeeze().item())
        
        agreeing_mask = ((probs[0] > 0.5) == pred)
        ...
        print(f"  Models agreeing with decision: {len(agreeing_indices)}/{len(model_names)}")
        ...
        if idx < num_gradcam:
            try:
                combined_cam = None
                # Average GradCAM of models that agreed with the final decision
                if len(agreeing_indices) > 0:
                    for i in agreeing_indices:
                        ...
                        # Use the probability for backprop
                        score = torch.sigmoid(model_out.squeeze(0))
...
# ================== MAIN FUNCTION ==================
def main():
    parser = argparse.ArgumentParser(description="Soft Voting Ensemble Evaluation")
    ...
    if is_main:
        ...
        print("="*70)
        print("\nSkipping training phase (Soft Voting is rule-based).")
    ...
    ensemble_test_acc, prob_dist = evaluate_ensemble_final_ddp(
    ...
    if is_main:
        ...
        print(f"Soft Voting Accuracy: {ensemble_test_acc:.2f}%")
        ...
        final_results = {
            ...
            'ensemble': {
                ...
                'average_probabilities': {name: float(p) for name, p in zip(MODEL_NAMES, prob_dist)}
            },
            'improvement': float(ensemble_test_acc - best_single)
        }

        os.makedirs(args.save_dir, exist_ok=True)
        results_path = os.path.join(args.save_dir, 'final_results_soft_voting.json')
        with open(results_path, 'w') as f:
            json.dump(final_results, f, indent=4)
        print(f"\nResults saved: {results_path}")

        final_model_path = os.path.join(args.save_dir, 'soft_voting_model_info.json')
