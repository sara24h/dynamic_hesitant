...
def final_evaluation_and_report(model, loader, device, save_dir, model_name, args, is_main):
    ...
    for i, global_idx in enumerate(tqdm(test_indices, desc="Final Eval")):
        ...
        # منطق Probability Averaging: اعمال sigmoid روی لاجیت‌های خام و سپس میانگین‌گیری
...
class SimpleAveragingEnsemble(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_details: bool = False):
        # ایجاد دو تانسور: یکی برای احتمالات و یکی برای لاجیت‌های خام
        ...
        for i in range(self.num_models):
            ...
            # ذخیره لاجیت خام
            raw_logits[:, i] = out
            # اعمال sigmoid برای تبدیل لاجیت به احتمال و ذخیره در outputs
            outputs[:, i] = torch.sigmoid(out)

        # میانگین‌گیری در سطح احتمال (Probability Averaging)
        final_output = outputs.mean(dim=1)
        
        if return_details:
            weights = torch.ones(x.size(0), self.num_models, device=x.device) / self.num_models
            # برگرداندن raw_logits به جای outputs برای استفاده در ارزیابی نهایی
...
def load_pruned_models(model_paths: List[str], device: torch.device, is_main: bool) -> List[nn.Module]:
    ...
    if is_main:
        print(f"All {len(model_paths)} models loaded!\n")
    return models
