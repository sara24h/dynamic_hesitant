...
# حذف ایمپورت‌های مربوط به DDP و Distributed
...
def save_ensemble_checkpoint(save_path: str, ensemble_model: nn.Module, model_paths: List[str], 
    ...
    # در حالت تک GPU دیگر نیازی به بررسی .module نیست
    model_to_save = ensemble_model 
    checkpoint = {
...
@torch.no_grad()
def evaluate_single_model(model: nn.Module, loader: DataLoader, device: torch.DeviceObjType,
    ...
    # حذف بخش all_reduce مربوط به DDP
...
# ================== MAIN FUNCTION ==================
def main():
    parser = argparse.ArgumentParser(description="Majority Voting Ensemble Evaluation (HARD VOTING) - Single GPU")
    ...
    # اختیاری: انتخاب شماره GPU (پیش‌فرض 0)
    ...
    # تنظیم دستگاه به صورت ساده برای تک GPU
    ...
    try:
        # ارسال is_distributed=False به طور قطع
    ...
    # ارزیابی انسامبل (چون DDP نداریم، مستقیم ensemble را می‌دهیم)
    ...
    # ساخت دیتالودر کامل برای ارزیابی نهایی
