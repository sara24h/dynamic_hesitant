...
برای تبدیل این کد به یک **انسمبل ساده (Standard/Simple Ensemble)** که در آن خروجی مدل‌ها به صورت ساده میانگین گرفته می‌شود (بدون منطق فازی یا وزن‌دهی پویا)، تغییرات زیر را اعمال کردم:

1.  **حذف کلاس‌های پیچیده:** کلاس‌های `HesitantFuzzyMembership` و `FuzzyHesitantEnsemble` حذف شدند.
2.  **اضافه کردن کلاس `SimpleEnsemble`:** این کلاس خروجی تمام مدل‌ها را دریافت کرده و میانگین آن‌ها (Soft Voting/Averaging) را به عنوان خروجی نهایی برمی‌گرداند.
3.  **حذف بخش آموزش:** از آنجا که در انسمبل ساده معمولاً وزن‌ها ثابت و برابر هستند (یا نیازی به آموزش مرحله دوم نیست)، توابع مربوط به `train_hesitant_fuzzy` حذف و فراخوانی آن در `main` غیرفعال شد.
4.  **سازگاری با سایر توابع:** خروجی تابع `forward` در انسمبل جدید طوری تنظیم شد که با توابع ارزیابی و رسم نمودار (که انتظار ساختار خاصی داشتند) سازگار باشد.

کد کامل و اصلاح شده به شرح زیر است:
