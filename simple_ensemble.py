...
class SimpleAveragingEnsemble(nn.Module):
    ...
    def forward(self, x, return_details=False):
        ...
        stacked_out = torch.stack(outputs, dim=1)          # [B, N_models, 1]
        final_output = stacked_out.mean(dim=1)             # [B, 1]
...
def generate_visualizations_internal(model, test_loader, device, vis_dir, model_names, num_gradcam, num_lime):
    ...
    # Target layer (از مدل اول انسامبل)
...
# ================== MAIN ==================
def main():
    parser = argparse.ArgumentParser(description="Simple Averaging Ensemble + ROC + Visualizations")
    ...
    # آمار نرمال‌سازی (فقط به تعداد مدل‌ها برش می‌خورد)
    ...
    from model.ResNet_pruned import ResNet_50_pruned_hardfakevsreal   # مسیر import را با ساختار پروژه‌تان تطبیق دهید
    models = []
    ...
    correct, total = 0, 0
    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="Accuracy"):
    ...
    # ROC – نسخه اصلاح‌شده
    print("\nCollecting probabilities for ROC curve...")
    all_labels = []
    all_probs = []

    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="ROC probs"):
            images = images.to(device)
            outputs = ensemble(images)               # [B, 1]
    ...
    print(f"  → {len(all_labels)} samples collected")
    ...
    try:
        ...
        plt.title('ROC – Simple Averaging Ensemble')
        ...
        print("Error in ROC computation/plotting:")
        import traceback
        traceback.print_exc()
