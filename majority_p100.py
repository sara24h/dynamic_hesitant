...
# حذف شد: import torch.distributed as dist
# حذف شد: from torch.nn.parallel import DistributedDataParallel as DDP, DataParallel as DP
...
def save_ensemble_checkpoint(save_path: str, ensemble_model: nn.Module, model_paths: List[str], 
    ...
    # چون DDP حذف شده، نیازی به چک کردن module نیست
    checkpoint = {
...
def evaluate_ensemble_final(model, loader, device, name, model_names):
    ...
    # حذف شد: بخش dist.all_reduce چون فقط روی یک GPU ران می‌شود
