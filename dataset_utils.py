...
class CustomGenAIDataset(Dataset):
    ...
    def __init__(self, root_dir, fake_classes, real_class, transform=None):
        ...
        if os.path.exists(real_path):
            ...
            print(f"[Warning] Path not found: {real_class}")
...
def get_sample_info(dataset, index):
    ...
    else: raise AttributeError("Cannot find samples in dataset")
...
def prepare_dataset(base_dir: str, dataset_type: str, seed: int = 42):
    ...
    if dataset_type == 'custom_genai':
        ...
        train_indices, val_indices, test_indices = create_standard_reproducible_split(full_dataset, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, seed=seed)
        ...
        if not all(os.path.exists(os.path.join(dataset_dir, f)) for f in folders):
            ...
            if all(os.path.exists(os.path.join(possible_sub_dir, f)) for f in folders): dataset_dir = possible_sub_dir
            else: raise FileNotFoundError(f"Could not find dataset folders {folders} in {base_dir}")
...
def create_dataloaders(base_dir: str, batch_size: int, num_workers: int = 0,
    ...
    # ... [بدون تغییر - دقیقاً همان کدی که شما فرستادید] ...
    if is_main:
        print("="*70)
    ...
    if dataset_type == 'wild':
        ...
        for split in splits:
            ...
            if is_main: print(f"{split.capitalize():5}: {path}")
        ...
        if is_main:
            print(f"\nDataset Stats:\n Total: {len(full_dataset):,} images\n Train: {len(train_indices):,} ({len(train_indices)/len(full_dataset)*100:.1f}%)\n Valid: {len(val_indices):,} ({len(val_indices)/len(full_dataset)*100:.1f}%)\n Test: {len(test_indices):,} ({len(test_indices)/len(full_dataset)*100:.1f}%)")
            if hasattr(full_dataset, 'class_to_idx'): print(f" Class → Index: {full_dataset.class_to_idx}\n")
...
if __name__ == '__main__':
    ...
    try:
        train_loader, val_loader, test_loader = create_dataloaders(
            base_dir=path_to_dataset, batch_size=32, num_workers=4, 
            dataset_type='real_fake_dataset', is_distributed=False, seed=42
        )
