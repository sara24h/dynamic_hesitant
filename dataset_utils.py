...
class CustomGenAIDataset(Dataset):
    """دیتاست قدیمی (جهت سازگاری با نسخه‌های قبلی)"""
    ...
    def __init__(self, root_dir, fake_classes, real_class, transform=None):
        ...
        # نکته مهم: اگر مدل‌های شما برعکس آموزش دیده‌اند، جای 0 و 1 را اینجا عوض کنید
        # فرض استاندارد: Fake=0, Real=1
...
class NewGenAIDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        if len(self.samples) == 0:
            print("[Error] No images found! Please check folder names ('real' or 'fake').")
...
def create_standard_reproducible_split(dataset, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, seed=42):
    ...
    # Handling cases where dataset might not have explicit labels attribute but has samples
    try:
        labels = [dataset.samples[i][1] for i in indices]
    except AttributeError:
        # Fallback if samples structure is different
...
def prepare_dataset(base_dir: str, dataset_type: str, seed: int = 42):
    ...
    if dataset_type == 'custom_genai':
        ...
        full_dataset = CustomGenAIDataset(base_dir, fake_classes=fake_folders, real_class=real_folder, transform=temp_transform)
...
def worker_init_fn(worker_id):
    """Initialize workers with proper seeds for reproducibility"""
...
def create_dataloaders(base_dir: str, batch_size: int, num_workers: int = 0,
    ...
    if dataset_type == 'wild':
        ...
        # --- CORRECTED: Added seed=seed and drop_last settings ---
        train_sampler = DistributedSampler(datasets_dict['train'], shuffle=True, seed=seed, drop_last=True) if is_distributed else None
        val_sampler = DistributedSampler(datasets_dict['valid'], shuffle=False, seed=seed, drop_last=False) if is_distributed else None
        test_sampler = DistributedSampler(datasets_dict['test'], shuffle=False, seed=seed, drop_last=False) if is_distributed else None
        ...
        # --- CORRECTED: Added seed=seed and drop_last settings ---
        train_sampler = DistributedSampler(train_dataset, shuffle=True, seed=seed, drop_last=True) if is_distributed else None
        val_sampler = DistributedSampler(val_dataset, shuffle=False, seed=seed, drop_last=False) if is_distributed else None
        sampler_test = DistributedSampler(test_dataset, shuffle=False, seed=seed, drop_last=False) if is_distributed else None
