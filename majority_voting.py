...
if __name__ == "__main__":
    main()
```کد زیر اصلاح شده است. من منطق `final_evaluation_unified` (که در کد `fuzzy_hesitant` استفاده کردم) را در اینجا پیاده‌سازی کردم و `main` را اصلاح کردم تا هم McNemar Log و هم فایل‌های ROC (JSON/TXT) تولید شوند.

تغییرات کلیدی:
1.  اضافه شدن تابع `final_evaluation_unified` که داده‌ها را تک‌به‌تک می‌خواند و هم McNemar و هم ROC را پر می‌کند.
2.  اصلاح بخش `main` برای ساختن `test_loader_full` و فراخوانی تابع ارزیابی جدید.
3.  اطمینان از اینکه فایل `roc_data_test.json` دارای فیلدهای `positive_count` و `negative_count` باشد.
