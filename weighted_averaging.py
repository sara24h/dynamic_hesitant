...
class WeightedAverageEnsemble(nn.Module):
    def __init__(self, models: List[nn.Module], means: List[Tuple[float]],
        ...
        # Initialize weights uniformly (1 / num_models)
    ...
    def forward(self, x: torch.Tensor, return_details: bool = False):
        ...
        # Run all models (Standard Weighted Averaging uses all models)
        ...
        if return_details:
            # Returning None for memberships to keep interface consistent
...
def train_weighted_ensemble(ensemble_model, train_loader, val_loader, num_epochs, lr,
    ...
    if is_main:
        ...
        print("="*70)
        print(f"Trainable params: {sum(p.numel() for p in [weights_param]):,} (Only Weights)")
    ...
    for epoch in range(num_epochs):
        ...
        if is_main:
            ...
            print(f"\n{'Current Model Weights:':^70}")
            print(f"{'-'*70}")
            for i, name in enumerate(model_names):
                w_val = current_weights[i].item()
                print(f"  {i+1:2d}. {name:<30}: {w_val:.6f} ({w_val*100:.2f}%)")
            print(f"{'-'*70}")
...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True,
                       choices=['wild', 'real_fake', 'hard_fake_real', 'real_fake_dataset', 'uadfV'])
