...
def get_sample_info(dataset, index):
    """Unified function to get sample info from any dataset type"""
...
def prepare_dataset(base_dir: str, dataset_type: str, seed: int = 42):
    """Unified dataset preparation function - NO REDUNDANCY"""
...
class GradCAM:
    """Optimized GradCAM implementation"""
...
class HesitantFuzzyMembership(nn.Module):
    """Hesitant Fuzzy Membership Generator"""
...
class FuzzyHesitantEnsemble(nn.Module):
    """Optimized Fuzzy Hesitant Ensemble with vectorized operations"""
    ...
    def _compute_mask_vectorized(self, final_weights: torch.Tensor, 
                                  avg_hesitancy: torch.Tensor) -> torch.Tensor:
        """Vectorized mask computation - NO LOOPS"""
        ...
        # Create mask for active models
        mask = (cum_weights <= self.cum_weight_threshold).float()
        mask[:, 0] = 1.0  # Always include at least one model
        
        # For high hesitancy samples, use all models
    ...
    def forward(self, x: torch.Tensor, return_details: bool = False):
        ...
        # Compute mask using vectorized operation
        ...
        # Get active models across entire batch
...
def create_dataloaders(base_dir: str, batch_size: int, num_workers: int = 2, 
    ...
    """Optimized dataloader creation - NO REDUNDANCY"""
    ...
    if dataset_type == 'wild':
        # Wild dataset - direct folder structure
        ...
        # Other datasets - use unified preparation function (NO REDUNDANCY!)
        if is_main:
...
def evaluate_single_model_ddp(model: nn.Module, loader: DataLoader, device: torch.device, 
    ...
    """Evaluate single model with DDP"""
    model.eval()
...
def evaluate_accuracy_ddp(model, loader, device):
    """Fast accuracy evaluation for validation"""
    model.eval()
    correct = 0
    total = 0
...
def generate_visualizations(ensemble, test_loader, device, vis_dir, model_names, 
    ...
    # Create directories
    dirs = {
        'gradcam': os.path.join(vis_dir, 'gradcam'),
        'lime': os.path.join(vis_dir, 'lime'),
        'combined': os.path.join(vis_dir, 'combined')
    }
    ...
    vis_indices = random.sample(range(total_samples), min(max(num_gradcam, num_lime), total_samples))
